...
This vignette will consider the following single example prompt:

```{r}
input <- "Package that works with dates and times in tidy format"
```

The expected result from that prompt is [the `lubridate`
package](https://lubridate.tidyverse.org/). By default, the object returned
from [the `pkmgatch_similar_pkgs()`
