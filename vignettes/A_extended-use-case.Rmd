...
or large language models ([Phind](https://www.phind.com/search/cmbqhybqt0000206imod5zpqo), [Perplexity](https://www.perplexity.ai/search/r-package-to-return-web-search-9mTr_z_eT2iqkKutaU3YYA)).
...
to generate three sets of inverse document frequencies: of all package code, and of all text both
with and without text from individual function descriptions. Matches with other
packages are based on combinations of matches with these three data sets
(as also explained in [the _How does pkgmatch work?_
...
text and code. Moreover, the only two packages matched on both code and text
are `crul` itself, and the highly-similar [`httr2`
package](https://httr2.r-lib.org/).
