...
pkgmatch can also be used when writing R functions, to answer the question of
which R packages may already exist which do what I am trying to code. The
easiest way to illustrate this is with a concrete example:
...
first converting it to a single character string:

```{r fn-search, eval = FALSE}
s <- paste0 (deparse (search_api_results), collapse = "\\n")
pkgmatch_similar_pkgs (s, corpus = "cran")
```
```{r fn-search-out, echo = FALSE}
c ("httping", "httr", "request", "httpcode", "httr2")
```

And those are all very directly related to our functional input. We can also
