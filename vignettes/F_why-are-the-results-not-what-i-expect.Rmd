...
> The `pkgmatch` package finds packages, as well as individual
...
specified corpora. Because all embeddings are approximate, matching is also
...
as expected. In contrast, consider the following example in which we expect
...
matches. The following sub-sections re-generate full package embeddings for
...
for the two corpora used here, rOpenSci and CRAN, are generated from five
...
LMs are approximate, and has demonstrated the consequences of those approximations.
