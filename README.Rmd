...
[![CRAN Downloads](https://cranlogs.r-pkg.org/badges/grand-total/pkgmatch?color=orange)](https://cran.r-project.org/package=pkgmatch)
<!-- badges: end -->

# pkgmatch

A tool to help find R packages by matching packages either to a text
description, or to entire packages. Can find matching packages either from
rOpenSci's [suite of packages](https://ropensci.org/packages), or from all
packages currently on [CRAN](https://cran.r-project.org).

## Installation

The easiest way to install this package is via the
[associated
...
to an R package, and finds matching packages based on text and code matching
algorithms.
...
path to a package directory, or the name of an installed package. For any
locally-installed packages, you can just use the name of the package:
```{r, similar-pkgs-installed}
pkgmatch_similar_pkgs (input = "httr2", corpus = "cran")
```

Alternatively, the `input` parameter can be a local path to a package
repository. To demonstrate that, the following code downloads a `.tar.gz` file
of the same `httr2` package from CRAN:
...
```{r pkgmatch-similar-tarball}
pkgmatch_similar_pkgs (path, corpus = "cran")
```

Websites of packages or functions returned by either [the `pkgmatch_similar_pkgs()`](https://docs.ropensci.org/pkgmatch/reference/pkgmatch_similar_pkgs.html)
...
TRUE`, or by storing the return value of these functions
...
- [_Data caching and
updating_](https://docs.ropensci.org/pkgmatch/articles/B_data-caching-and-updating.html)
...
- The [`pkgsearch` package](https://r-hub.github.io/pkgsearch) to "_search R
packages on CRAN_" using CRAN metadata.
- The [`packagefinder`
package](https://cran.r-project.org/web/packages/packagefinder/index.html) for
matching to DESCRIPTION entries of CRAN packages.
